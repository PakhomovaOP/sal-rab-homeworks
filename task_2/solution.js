function calcShipping(sum, min, shipping) {
    ...
    // Задание №2.1. Рассчитать доставку
     ...
     if ((productsSum == 0) || (productsSum >= freeShippingMinSum))// если productsSum равно 0,
        {shippingSum = 0;
            console.log ('В корзине ', productsSum, 'Доставка бесплатная!');
        }// то shippingSum присвоить значение 0
       else { shippingSum = shippingPrice;
            console.log ('В корзине ', productsSum, 'Стоимость доставки', shippingSum);
       }
...
function calcDiscount(sum, min, discount) {
    ...
    {discountSum = (discountPart / 100) * productsSum;
        console.log ('Ваша скидка', discountSum)
     } else 
     {discountSum = 0;
...
function calcInvoice({sum, discountMinSum, discountPart, shippingFreeMinSum, shippingPrice}) {
    ...
    console.log ('Сумма заказа с учетом скидки',totalSum);// создайте переменную totalSum
    ...
    console.log ('Сумма заказа с учетом скидки и доставки',totalSum);
